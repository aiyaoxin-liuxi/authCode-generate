...
$(function(){
	$("#addSub").click(function(){
		if(checkFrom()){
            $.ajax({  
                url : "${basePath}/authCode/addAuthInfo",  
                type : "POST",
...
<body>
	...
	<table border="1px">
		...
		<tr>
			<td>使用公司名称</td>
			<td><input type="text" id="useCompanyName" name="useCompanyName" /></td>
		</tr>
		<tr>
			<td>加密类型</td>
			<td>
			     <select id="keyType" name="keyType">
			         <option value="">请选择</option>
                        <c:forEach items="${keyTypeMap }" var="eachone">
                            <option value="${ eachone.key}" >${ eachone.value}</option>
                        </c:forEach>
			     </select>
			</td>
		</tr>
		<tr>
			<td>起始时间</td>
			<td>
                <input id="wStartDate" placeholder="起始日期" class="laydate-icon data" onFocus="var endDate=$dp.$('endDate');WdatePicker({onpicked:function(){endDate.focus();$dp.$('startDate').value=this.value;},maxDate:'#F{$dp.$D(\'endDate\')}',minDate:'%y-%M-{%d+1}',readOnly:true})"/>
		...
		<tr>
			<td>到期时间</td>
			<td>
				<input id="wEndDate" placeholder="结束日期" class="laydate-icon data" onFocus="WdatePicker({onpicked:function(){$dp.$('endDate').value=this.value;},minDate:'#F{$dp.$D(\'startDate\')}',readOnly:true})" />
	...
	<input type="button" id="addSub" value="添加">
	</form>
	<br>
	<br>
	<br>
