...
/**
 * 验证公用方法：1、非空验证；2、不包含空格；3、系统长度不超过20字；4、不能存在非法字符
 * 传入参数，param1：验证的参数；param2：错误提示中文开头
 */
function checkCommon(val, alertName){
    if(val == '' || val == null){
        alert(alertName + "不能为空");
        return false;
    }
    if(checkTextSpace(val)){
        alert(alertName + "不能包含空格");
        return false;
    }
    if(overLengthStr(val, 20)){
        alert(alertName + "不能超过20字");
        return false;
    }
    if(specialStr(val)){
    	alert(alertName + "不能存在非法字符");
    	return false;
    }
    return true;
}
